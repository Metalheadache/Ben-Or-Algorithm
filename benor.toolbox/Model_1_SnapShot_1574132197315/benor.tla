...
\*Agreement == \A i,j \in Procs: (~(decided[i] = -1) /\ ~(decided[j] = -1)) =>
\*             (decided[i] = decided[j])

\*MinorityReport == Termination => \E i \in Procs: decided[i] = 0
\*Termination == <>(\A self \in Procs: k[self] > MAXROUND)

============================================================================
Agreement: If both i node and j node are decided,
           then they should decide the save value.
...
Termination says that all node terminated eventually.
             Termination is true since MAXROUND is finite.

BaitProgress: It is not possible for all processes to decide and terminate,
              BaitProgress is false.

MinorityReport: set N = 4 and vp = <0,1,1,1>, we can check 
                MinorityReport is true.
